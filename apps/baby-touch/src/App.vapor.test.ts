...
function setSurfaceRect(wrapper: VaporTestWrapper, width = 200, height = 100): void {
  const surface = wrapper.find('[data-testid="baby-touch-surface"]');
  setElementRect(surface.element, width, height);
}

function setOrientationViewportRect(
...
async function startGameFromBackground(wrapper: VaporTestWrapper, index = 0): Promise<void> {
