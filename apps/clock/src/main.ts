 ...
 * first-party widget loaders resolve these by `exportName` (see the `widgets`
 * descriptor in `src/core/apps/firstParty/registry.ts`), so the menubar,
 * wallpaper, and mobile widget surfaces all render from this one published
 * module. Vue and the `@daopk/*` runtime surface are external (vite.config.ts).
 */
