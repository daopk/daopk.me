...
const stubAppIcon = defineVaporComponent(() => {
  const icon = document.createElement("img");
  icon.alt = "";
  return icon;
});

const PdfPreviewStub = defineVaporComponent(() => {
  const preview = document.createElement("div");
  preview.className = "pdf-preview-stub";
  preview.textContent = "PDF preview";
  return preview;
});
