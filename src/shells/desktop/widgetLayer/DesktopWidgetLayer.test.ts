  ...
  default: defineComponent({
    ...
    setup(props, { emit }) {
      return () =>
        h("div", {
          class: "slot-stub",
          "data-widget-id": (props.manifest as WidgetManifest).id,
          "data-grid-x": (props.placement as { gridX: number; gridY: number }).gridX,
          "data-grid-y": (props.placement as { gridX: number; gridY: number }).gridY,
          "data-host-w": (props.hostSize as { width: number; height: number }).width,
          "data-host-h": (props.hostSize as { width: number; height: number }).height,
          onClick: () => emit("drop", (props.manifest as WidgetManifest).id, 12, 8),
        });
    },
  }),
}));
