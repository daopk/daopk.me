...
function makeManifest(overrides: Partial<WidgetManifest> = {}): WidgetManifest {
  const Stub = defineVaporComponent(
    () => {
      const element = document.createElement("div");
      element.className = "widget-stub";
      element.dataset.testid = "widget-stub";
      element.textContent = "stub";
      return element;
    },
    { name: "WidgetStub" },
  );
...
describe("DesktopWidgetSlot (M3.7)", () => {
  ...
  describe("drag lifecycle", () => {
    ...
    it("lets interactive widget controls receive click when no drag starts", async () => {
      const onClick = vi.fn();
      const ButtonWidget = defineVaporComponent(
        () => {
