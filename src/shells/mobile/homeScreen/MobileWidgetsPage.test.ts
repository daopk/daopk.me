...
function registerStub(id: string, size: "sm" | "md" | "lg"): () => void {
  return kernel.widgets.register({
    ...
    size,
    component: () =>
      Promise.resolve({ default: defineComponent({ render: () => h("div", { class: id }) }) }),
  });
}
...
describe("MobileWidgetsPage (M1.4 commit B)", () => {
  ...
  it("does NOT render widgets from other surfaces (filter correctness)", () => {
    kernel.widgets.register({
      ...
      size: "sm",
      component: () => Promise.resolve({ default: defineComponent({ render: () => h("div") }) }),
    });
        ...
        widgets: [
          {
            ...
            component: () =>
              Promise.resolve({
                default: defineComponent({ render: () => h("div", { class: "lunar" }) }),
              }),
          },
        ],
      });
