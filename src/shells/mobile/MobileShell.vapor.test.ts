...
vi.mock("~/components/wallpaper/Wallpaper.vue", async () => {
  const { defineVaporComponent } = await import("vue");
  return {
    default: defineVaporComponent(() => {
      const wallpaper = document.createElement("div");
      wallpaper.className = "wallpaper-stub";
      return wallpaper;
    }),
  };
});
...
async function finishSwitcherTransition(): Promise<void> {
...
describe("MobileShell (v2 — back-as-suspend)", () => {
  ...
  it("does not emit `launch` on its own root (HomeScreen → MobileShell only)", async () => {
    const wrapper = mount(MobileShell);
    const icon = wrapper.find(".home-icon");
  ...
  it("mounts a foreground AppView after an icon-driven launch (M1.2 integration)", async () => {
    ...
    await wrapper.find(".home-icon").trigger("click");
    await flushPromises();
    await nextTick();
  ...
  it("close from a hosted app dismisses the frame and kills the process", async () => {
    const Probe = defineVaporComponent(() => {
      ...
      button.dataset.testid = "close-app";
      button.textContent = "Close";
      button.addEventListener("click", () => chrome?.close?.());
      return button;
    });
  ...
  it("hide from a foregrounded app returns home instead of foregrounding another alive app", async () => {
    ...
    const wrapper = mount(MobileShell, { attachTo: document.body });
    const icons = wrapper.findAll(".home-icon");
  ...
  it("closes the AppSwitcher when it emits `close`", async () => {
    ...
    await nextTick();
    await finishSwitcherTransition();
